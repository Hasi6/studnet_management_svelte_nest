...
export class EventsRepo {
  ...
  public getEventById = async (_id: string) => {
    try {
      return await this.eventsRepository
        .findById(_id)
        .populate('participants', ['image', 'username', '_id', 'email']);
    } catch (err) {
      throw new InternalServerErrorException();
    }
  };
  ...
  public addParticipants = async (_id: string, userId: string) => {
    try {
      await this.eventsRepository.updateOne(
        { _id },
        { $addToSet: { participants: { $each: [userId] } } },
      );
      return true;
    } catch (err) {
      throw new InternalServerErrorException();
  ...
  public deleteEventById = async (_id: string) => {
    try {
      await this.eventsRepository.deleteOne({ _id });
      return true;
    } catch (err) {
      throw new InternalServerErrorException();
    }
  ...
  public deleteEventsByUser = async (user: string) => {
    try {
      await this.eventsRepository.deleteMany({ user });
      return true;
    } catch (err) {
      throw new InternalServerErrorException();
    }
